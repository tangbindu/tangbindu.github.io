...
const tools = {
    ...
    toLogicPixel(point, ratio, scale, px, py) {
        const x = (point.x * ratio) / scale - px;
        const y = (point.y * ratio) / scale - py;
        return {
            x: tools.toInt(x),
            y: tools.toInt(y),
        };
    ...
    resetRectPoint(rectShape) {
        const tempPoints = rectShape.points.sort((a, b) => a.y - b.y);
        rectShape.points = tempPoints.slice(0, 2).sort((a, b) => a.x - b.x)
            .concat(tempPoints.slice(2, 4).sort((a, b) => b.x - a.x));
    },
    // 获取中心点
    ...
    expandValue(value) {
        ...
        tools.expandValueTimmer = setTimeout(() => {
